 ...
 * CanvasFeaturedInput
 *
 * Custom input component for the `featuredImages` object type.
 * Wraps the same canvas editing experience as CanvasGalleryInput
 * but adds a primary image selector — the editor can click a
 * button on each image to mark it as the one the project title
 * anchors beneath on the homepage.
 *
...
import {EyeOpenIcon, EyeClosedIcon, PinIcon} from '@sanity/icons'
import {
...
// ─────────────────────────────────────────────────────────────
// Draggable Image (with resize handle + primary badge)
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
// Controls Panel (with primary image toggle)
// ─────────────────────────────────────────────────────────────
...
function ControlsPanel({image, imageIndex, isPrimary, onChange, onSetPrimary}: ControlsPanelProps) {
  return (
    <Card padding={3} radius={2} border tone="default">
      <Stack space={4}>
        <Flex align="center" justify="space-between">
          ...
          <Tooltip
            content={
              <Box padding={2}>
                <Text size={1}>
                  {isPrimary
                    ? 'This is where the project title will appear'
                    : 'Set this image as the title anchor'}
                </Text>
              </Box>
            }
            placement="top"
          >
            <Button
              mode={isPrimary ? 'default' : 'ghost'}
              tone={isPrimary ? 'positive' : 'default'}
...
// ─────────────────────────────────────────────────────────────
// Canvas Featured Input (main component)
// ─────────────────────────────────────────────────────────────

export function CanvasFeaturedInput(props: ObjectInputProps<FeaturedImagesValue>) {
  ...
  const primaryImage = value?.primaryImage ?? 0
  ...
  const handleSetPrimary = useCallback(
    (index: number) => {
      onChange(set(index, ['primaryImage']))
    },
    [onChange],
  )
    ...
    <Stack space={4}>
      ...
      {selectedImage && selectedIndex !== -1 && (
        <ControlsPanel
          ...
          isPrimary={selectedIndex === primaryImage}
          onChange={handleSliderChange}
          onSetPrimary={handleSetPrimary}
        />
      )}

      {/* Image count + primary indicator */}
      <Flex align="center" justify="space-between">
        <Text size={1} muted>
          {images.length}/3 images · Primary: image {primaryImage + 1}
        </Text>
        {selectedKey && (
          <Text size={0} muted>
